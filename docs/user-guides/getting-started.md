...
Developer fixtures live separately under `src/test/fixtures/` and `tests/`. Those are imported by tests, not loaded through production product import dialogs. See [Sample Data and Fixtures](../development/sample-data-and-fixtures.md) for the full map.
