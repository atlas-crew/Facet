...
- the entitlement status is `paid` or `active`
...
- the entitlement status is `expired`
- the entitlement status is `refunded`

Failure semantics:

- missing entitlement or missing feature -> `upgrade_required`
- expired entitlement -> `access_expired`
- refunded entitlement -> `billing_issue`
...
- reject hosted AI calls when entitlement is missing, expired, or refunded
